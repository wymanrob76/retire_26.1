// montecarlo.js — AuraRetire stochastic simulation
// Runs N independent projections with randomised annual returns + inflation.
// Returns success rate and percentile statistics.

import { phaseRate } from './projection.js';
...
// ── Main entry point ─────────────────────────────────────────────────────────
export function runMonteCarlo(A) {
  ...
  for (let i = 0; i < N; i++) {
    const r = simulate(A, accSd, distSd, inflSd);
...
function simulate(A, accSd, distSd, inflSd) {
  ...
  while (age < retireAge) {
    ...
    // Randomise returns (clamp at -50% floor)
    ...
    const matchCap   = A.k401.employerMatchCapPercent / 100;
    const matchRate  = A.k401.employerMatchPercent    / 100;

    k401Bal = k401Bal * Math.pow(1 + k401Rate, yr)
            + salary * k401Pct * yr
            + salary * Math.min(k401Pct, matchCap) * matchRate * yr;
  ...
  const userSSYearly = (userSSAge === 67
  ...
  for (let da = retireAge; da <= lifeExp; da++) {
    ...
    const thisInflation = Math.max(0, randn(baseInflation, inflSd));
    const thisPortRet   = Math.max(-0.5, randn(portReturn, distSd));

    const inflatedSpend = baseSpend * Math.pow(1 + thisInflation, yearsFromNow);

    let ssIncome = 0;
    if (da >= userSSAge) {
      ssIncome += userSSYearly * Math.pow(1 + ssColaRate, da - userSSAge);
    }
  ...
  return {
    ...
    finalWealth: Math.max(0, portfolioBal) + ltcBal,
  };
}
